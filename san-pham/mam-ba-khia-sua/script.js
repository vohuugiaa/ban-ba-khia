document.addEventListener('DOMContentLoaded', () => {
    ...
    btnSubmitOrder.addEventListener('click', () => {
        // --- CẤU HÌNH APPS SCRIPT URL TẠI ĐÂY ---
        const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz0J4t6aLygpZNEp_Rx0HPOF1AWuRQFAUY1e7ll48Q5dzeq24uRBOg3Pe7NGAZlcFgE1g/exec';
        
        if (!APPS_SCRIPT_URL) {
            // Fake request for demo if URL is empty
            btnSubmitOrder.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Đang xử lý...';
            btnSubmitOrder.disabled = true;
            setTimeout(() => {
                closeModal();
                showToast('Đặt hàng thành công! (Chế độ Demo - Chưa gắn link Google Sheet)');
        ...
        const orderData = {
            product: `${selectedWeightText}, ${selectedSpiceText}`,
            quantity: quantity,
            name: cusName.value.trim(),
        ...
        fetch(APPS_SCRIPT_URL, {
            method: 'POST',
            mode: 'no-cors', // Bỏ qua CORS error khi gọi script google
            headers: {
        ...
        .then(() => {
            closeModal();
            showToast('Đặt hàng thành công! Shop sẽ liên hệ sớm nhất.');
        ...
        .catch(err => {
            console.error(err);
            showCustomAlert('Có lỗi xảy ra khi gửi đơn hàng. Vui lòng thử lại sau!');
            btnSubmitOrder.innerHTML = 'Xác nhận Đặt Hàng';
            btnSubmitOrder.disabled = false;
        });
    });
